...
{
    public class AirdropWithdrawWalletWindowViewModel : ViewModelBase
    {
        ...
        private async void PreviewProcess()
        {
            ...
            string totalWallets = $"{newWallet} wallet's Layer 1.";
            ...
            // Handle null tokenAddress scenario  
            var firstTokenAddress = allNfts.FirstOrDefault()?.FirstOrDefault()?.tokenAddress;
            if (string.IsNullOrEmpty(firstTokenAddress))
            {
                Log = "Transfer fees can't be calculated right now...";
                return;
            }

            var transferFees = (decimal.Parse((await LoopringService.GetNftWithdrawOffChainFee(settings.LoopringApiKey, settings.LoopringAccountId, 10, firstTokenAddress)).fees.First(x => x.token == LoopringFeeSelectedOption).fee) / 1000000000000000000) * totalTransfersNumber;
        ...
        private async void NftWithdrawWallet()
        {
            ...
            // Filter the original structure of allNfts to only include items where deploymentStatus is "DEPLOYED"
            allNfts = allNfts.Select(innerList => innerList.Where(datum => datum.deploymentStatus == "DEPLOYED").ToList())
            ...
            /*
             * NOT NEED PROBABLY AS WITHDRAW IS STRAIGHT TO L1 
             *
             */
